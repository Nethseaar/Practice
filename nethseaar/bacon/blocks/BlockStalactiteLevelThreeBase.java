...
public class BlockStalactiteLevelThreeBase extends Block {

	@SideOnly(Side.CLIENT)
    private Icon stalactiteTop;
	private Icon stoneBottom;
	
        public BlockStalactiteLevelThreeBase(int id, Material material) {
                super(id, material);
            	setHardness(3.0F);
        		setResistance(5.0F);
    			...
    			if (l < 3)
    			{
    				...
    				if (i1 == 15)
    				{
    					if (par5Random.nextInt(3) == 1){
    						...
    						if (par5Random.nextInt(3) == 1){
    							par1World.setBlock(par2, par3, par4, BaconBlocks.stalactiteLevelTwoBase.blockID);
    						}
    					}
    					else{
